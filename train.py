...
from utils.unet_training import get_lr_scheduler, set_optimizer_lr, weights_init
from utils.callbacks import LossHistory, EvalCallback
from utils.dataloader import UnetDataset, unet_dataset_collate
from utils.utils import show_config, seed_everything, worker_init_fn
...
if __name__ == "__main__":
    ...
    eval_flag = True
    eval_period = 1
    VOCdevkit_path = r'G:\图像处理\第五篇_损伤修复\数据集\新新数据集\VOCdevkit'
    dice_loss = True
    focal_loss = True
    ...
    model = UNet(num_classes = num_classes).train()
    weights_init(model)
    if model_path != '':
        if local_rank == 0:
    ...
    with open(os.path.join(VOCdevkit_path, r"VOC2007/ImageSets/Segmentation/train.txt"), "r") as f:
        train_lines = f.readlines()
    with open(os.path.join(VOCdevkit_path, r"VOC2007/ImageSets/Segmentation/val.txt"), "r") as f:
    ...
    if local_rank == 0:
        ...
        seed = 11
        rank = local_rank
        worker_init_fn_seeded = partial(worker_init_fn, rank=rank, seed=seed)
